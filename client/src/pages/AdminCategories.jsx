...
const AdminCategoriesContent = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      axios.get(`${API_BASE_URL}/api/categories/admin/all`).then((res) => res.data),
  });

  const saveCategoryMutation = useMutation({
    mutationFn: (payload) => {
      if (editingId) {
        return axios.put(`${API_BASE_URL}/api/categories/${editingId}`, payload);
      }

      return axios.post(`${API_BASE_URL}/api/categories`, payload);
    },
    onSuccess: (response) => {
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (categoryId) => axios.delete(`${API_BASE_URL}/api/categories/${categoryId}`),
    onSuccess: (response) => {
