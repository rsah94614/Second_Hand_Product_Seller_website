...
export default function AdminAuditScreen() {
  ...
  const {
    ...
    queryKey: ["admin-audit", debouncedTarget],
    initialPageParam: null as string | null,
    queryFn: async ({ pageParam }) => {
      const params: Record<string, string> = { limit: "40" };
      if (debouncedTarget.trim()) params.targetType = debouncedTarget.trim();
    ...
    <Screen className="bg-slate-50 dark:bg-slate-950" safeAreaTop={false}>
      ...
      {status === "error" ? (
        <View className="flex-1 items-center justify-center p-6">
          ...
          <Text className="mt-4 text-[16px] font-outfit-m text-slate-900 dark:text-white">Failed to load logs</Text>
          ...
          <Pressable onPress={() => queryClient.invalidateQueries({ queryKey: ["admin-audit"] })} className="mt-6 rounded-xl bg-primary-600 px-6 py-2.5">
