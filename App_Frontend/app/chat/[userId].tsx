...
const generateIdempotencyKey = (): string => {
  // React Native doesn't have crypto.randomUUID in all versions; use a reliable fallback
...
// ── In-memory offline queue (survives disconnect/reconnect within session) ────
type QueuedMessage = {
  receiver: string;
  content: string;
...
// Module-level queue so it persists across re-renders
...
function ChatThreadContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Send read acknowledgment for all messages from partner
      if (socket) {
  ...
  // ── Drain offline queue when socket reconnects ────────────────────────────
  useEffect(() => {
    if (!socket || !isConnected || offlineQueue.length === 0) return;

    // Drain a copy and clear the module-level queue
    const pending = offlineQueue.splice(0, offlineQueue.length);

    pending.forEach((item) => {
      socket.emit(
        "send_message",
        { receiver: item.receiver, content: item.content, idempotencyKey: item.idempotencyKey },
  ...
  const send = () => {
    ...
    if (socket && isConnected) {
      socket.emit(
        "send_message",
        { receiver: partnerId, content: trimmed, idempotencyKey },
        (ack: { success: boolean; error?: string; duplicate?: boolean }) => {
          if (!ack?.success) {
            // Server rejected — remove optimistic message
            setMessages((prev) => prev.filter((m) => m._id !== optimisticMessage._id));
            Alert.alert("Send failed", ack?.error || "Failed to send message. Please try again.");
          }
        }
      );
    } else {
      // Offline: queue for delivery on reconnect
      offlineQueue.push({
        receiver: partnerId,
        content: trimmed,
        idempotencyKey,
  ...
  return (
    <Screen className="bg-slate-50 dark:bg-slate-950" safeAreaTop={false} safeAreaBottom={false}>
      ...
      ) : loadError ? (        <View className="flex-1 items-center justify-center px-8">
              ...
              renderItem={({ item: message }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View className={`mt-1 flex-row items-center gap-1 ${mine ? "justify-end mr-1" : "justify-start ml-1"}`}>
                      ...
                      {mine && (
                        <View className="flex-row items-center gap-0.5">
                          {message.read ? (
                            <>
                              <Ionicons name="checkmark-done" size={10} color="#3b82f6" />
                              <Text className="text-[9px] font-outfit-m text-blue-500">Read</Text>
                            </>
                          ) : message.delivered ? (
                            <>
                              <Ionicons name="checkmark-done" size={10} color="#64748b" />
                              <Text className="text-[9px] font-outfit-m text-slate-500 dark:text-slate-400">Delivered</Text>
                            </>
                          ) : (
                            <>
                              <Ionicons name="checkmark" size={10} color="#94a3b8" />
                              <Text className="text-[9px] font-outfit-m text-slate-400 dark:text-slate-500">Sent</Text>
                            </>
                          )}
                        </View>
                      )}
          ...
          <View className="border-t border-slate-200 bg-white px-3 py-2 pb-3 dark:border-slate-800 dark:bg-slate-900">
            ...
            <View className="flex-row items-end gap-2">
              ...
              <Pressable
                onPress={send}
                disabled={!text.trim()}
                className={`h-11 w-11 items-center justify-center rounded-full ${text.trim() ? "bg-primary-600 active:bg-primary-700 dark:bg-primary-500" : "bg-slate-200 dark:bg-slate-700"
                  }`}
              >
      ...
      <Modal visible={reportOpen} transparent animationType="slide" onRequestClose={() => setReportOpen(false)}>
        <View className="flex-1 justify-end bg-black/60">
          <DynamicKeyboardView style={{ flex: 1, justifyContent: "flex-end" }}>
