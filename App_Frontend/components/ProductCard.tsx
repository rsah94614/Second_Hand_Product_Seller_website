    ...
    <Link href={`/product/${product._id}` as never} asChild>
      <Pressable className="flex-1 overflow-hidden rounded-3xl bg-white dark:bg-slate-900 shadow-sm shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 active:scale-[0.98] transition-transform duration-200">
        ...
        <View className="p-3">
          ...
          {!product.isSold && user && (
            <Pressable
              className="mt-auto py-2 bg-slate-100 dark:bg-slate-800 rounded-xl items-center flex-row justify-center gap-1 active:bg-slate-200 dark:active:bg-slate-700"
              onPress={(e) => {
                e.preventDefault();
                e.stopPropagation();
                cartM.mutate();
              }}
              disabled={cartM.isPending}
            >
               <Ionicons name="cart-outline" size={14} color="#6366f1" />
               <Text className="text-[12px] font-outfit-sb text-primary-600 dark:text-primary-400">
                 {cartM.isPending ? "Adding..." : "Add to Cart"}
               </Text>
            </Pressable>
          )}
        </View>
